...
module.exports = function(opts) {
  return new Promise(function(resolve, reject) {
    ...
    var tag = opts.tag;
    var latestDir = path.normalize(file + '.deployed' + path.sep + tag);

    var abiFile = latestDir + path.sep  + 'abi.json';
    var addressFile = latestDir + path.sep  + 'address';

    var address, abi, contract, methodAbi, events;
    try {
      abi = JSON.parse(fs.readFileSync(abiFile).toString());
      address = opts.address || fs.readFileSync(addressFile).toString();
    } catch (e) {
      return reject(e);
    }
